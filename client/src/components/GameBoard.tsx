...
/** Taunt check that honors both the card's base keywords AND any Taunt
 * granted via an enchantment (e.g. Reinforcement, Prevent Destruction).
 * Silence suppresses both sources. */
function hasTauntKeyword(minion: BoardMinion, def?: CardDef): boolean {
  if (minion.isSilenced) return false;
  if (def?.keywords.includes('TAUNT')) return true;
  return minion.enchantments.some((e) => e.addedKeywords?.includes('TAUNT'));
}
...
// Custom hero power names keyed to the new class archetypes:
//   Scout / Catalyst / Operator / Guardian / Striker / Sentinel / Conduit / Dominion / Architect.
// Mechanics remain Hearthstone-classic (Steady Shot / Fireblast / etc.) —
// only the in-game name + flavor changes.
const HERO_POWER_DESC: Record<HeroClass, string> = {
  JIMMY:  'Precision Shot: Deal 2 damage to the enemy hero',
  IZZY:   'Catalyze: Deal 1 damage',
  ASTRID: 'Concealed Blade: Equip a 1/2 Wicked Knife',
...
// ─── Hero Archetype Display Names ───
// The mechanical archetype each hero plays (HS-classic remap). These are
// the user-facing class labels — "Jimmy the Scout" instead of "Jimmy the
// Hunter", etc. See project memory for the mechanical mapping.
export const HERO_ARCHETYPE: Record<HeroClass, string> = {
  JIMMY:   'Scout',      // Hunter mechanics
  IZZY:    'Catalyst',   // Mage mechanics
  ASTRID:  'Operator',   // Rogue mechanics
  TALA:    'Guardian',   // Priest mechanics
  ANDERS:  'Striker',    // Warrior mechanics
  AVA:     'Sentinel',   // Paladin mechanics
  LUCAS:   'Conduit',    // Shaman mechanics
  DES:     'Dominion',   // Warlock mechanics
  DEREK:   'Architect',  // Druid mechanics
  NEUTRAL: 'Neutral',
};

// ─── Hero Power PNG Icons (generated by scripts/generate-heroes-and-icons.py) ───
// All 9 heroes get a real icon; the SVG map below is kept as a fallback
// in case a PNG is missing at load time.
const HERO_POWER_PNGS: Partial<Record<HeroClass, string>> = {
  JIMMY:  '/hero-powers/JIMMY.png',
  IZZY:   '/hero-powers/IZZY.png',
  ASTRID: '/hero-powers/ASTRID.png',
  TALA:   '/hero-powers/TALA.png',
  ANDERS: '/hero-powers/ANDERS.png',
  AVA:    '/hero-powers/AVA.png',
  LUCAS:  '/hero-powers/LUCAS.png',
  DES:    '/hero-powers/DES.png',
  DEREK:  '/hero-powers/DEREK.png',
};

// ─── Hero Portrait PNGs (JIMMY/TALA/DEREK shipped, rest generated by
// scripts/generate-heroes-and-icons.py) ───
...
function BoardMinionCard({
  ...
  const hasTaunt = hasTauntKeyword(minion, def);
  ...
  return (
    <button
      ...
      `}
    >
      {/* Taunt stone shield — Hearthstone-style heater shield behind the oval */}
      {hasTaunt && (
        <svg
          viewBox="0 0 100 120"
        ...
        >
          <defs>
            <linearGradient id="taunt-stone-fill" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#b8b5ad" />
              <stop offset="45%" stopColor="#7a7770" />
              <stop offset="100%" stopColor="#3a3833" />
            </linearGradient>
            <linearGradient id="taunt-stone-hl" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#ffffff" stopOpacity="0.45" />
              <stop offset="50%" stopColor="#ffffff" stopOpacity="0.08" />
              <stop offset="100%" stopColor="#000000" stopOpacity="0.25" />
            </linearGradient>
          </defs>
          {/* Outer stone shield silhouette */}
          <path
            d="M 8 4 C 32 1, 68 1, 92 4 C 97 28, 96 58, 90 82 C 81 104, 66 118, 50 122 C 34 118, 19 104, 10 82 C 4 58, 3 28, 8 4 Z"
            fill="url(#taunt-stone-fill)"
            stroke="#1f1d18"
            strokeWidth="1.2"
          />
          ...
          <path
            d="M 8 4 C 32 1, 68 1, 92 4 C 97 28, 96 58, 90 82 C 81 104, 66 118, 50 122 C 34 118, 19 104, 10 82 C 4 58, 3 28, 8 4 Z"
            fill="url(#taunt-stone-hl)"
          />
          {/* Rivets */}
          <circle cx="12" cy="9" r="1.8" fill="#2a2824" stroke="#d0cdc4" strokeWidth="0.5" />
          <circle cx="88" cy="9" r="1.8" fill="#2a2824" stroke="#d0cdc4" strokeWidth="0.5" />
          <circle cx="6" cy="60" r="1.8" fill="#2a2824" stroke="#d0cdc4" strokeWidth="0.5" />
          <circle cx="94" cy="60" r="1.8" fill="#2a2824" stroke="#d0cdc4" strokeWidth="0.5" />
        </svg>
      )}
      {/* Rarity-colored ring */}
      ...
      {/* Divine Shield golden aura — layered halo around the oval */}
      {hasDivine && !isSilenced && (
        <div
          className="absolute inset-0 z-[3] pointer-events-none animate-divine-sparkle"
...
function DeckPile({
  ...
  const cb = CARD_BACK_STYLES[cardBackId ?? 'default'] ?? CARD_BACK_STYLES['default'];
  // Card-back sheet: use the player's selected back (image or gradient).
  const backStyle: React.CSSProperties = cb?.type === 'image'
    ? { background: 'transparent' }
    : { background: cb?.value ?? 'linear-gradient(135deg, #3b1e0e, #1c0f00)' };
  const borderClass = cb?.borderColor ?? 'border-amber-700';
  return (
    <div className="flex flex-col items-center gap-1.5">
      {/* Deck — uses player's chosen card back. Width-anchored at 64px;
           height flexes to fill available vertical space. */}
      <div
        className={`relative w-16 flex-1 min-h-[80px] max-h-[240px] rounded-lg border-2 ${borderClass} overflow-hidden flex items-center justify-center cursor-default group shadow-[0_4px_10px_rgba(0,0,0,0.5)]`}
        style={backStyle}
      ...
      >
        ...
        {count > 0 && (
          <>
            <div className={`absolute -top-0.5 -left-0.5 w-16 h-full rounded-lg border ${borderClass} opacity-30 pointer-events-none`} />
            {count > 5 && <div className={`absolute -top-1 -left-1 w-16 h-full rounded-lg border ${borderClass} opacity-20 pointer-events-none`} />}
      ...
      {graveyardCount > 0 && (
        <button
          ...
          className="relative w-10 h-5 rounded border border-stone-700 bg-stone-900/80 flex items-center justify-center cursor-pointer hover:border-stone-500 transition-colors group flex-shrink-0"
...
export default function GameBoard({
  ...
  // ─── Action toast — surfaces what's happening on the opponent side so
  // remote/AI moves are never silent. Watches PLAY/COMBAT/EFFECT log
  // entries tagged with the opponent's playerIndex and flashes the log
  // text above the opponent hero. Independent of the card showcase
  // (which only fires for actual card plays): a Totemic Call, attack, or
  // Life Tap that has no cardCode still toasts here. ───
  const [actionToast, setActionToast] = useState<{ text: string; mine: boolean; key: number } | null>(null);
  const toastLogIdRef = useRef<number>(-1);
  useEffect(() => {
    if (!gs.log || gs.log.length === 0) return;
    if (toastLogIdRef.current === -1) {
      toastLogIdRef.current = gs.log[gs.log.length - 1].id;
      return;
    }
    let latest: { text: string; mine: boolean; id: number } | null = null;
    for (const entry of gs.log) {
      if (entry.id <= toastLogIdRef.current) continue;
      if (entry.playerIndex === null) continue;
      const isAction = entry.category === 'PLAY' || entry.category === 'COMBAT' || entry.category === 'EFFECT';
      if (!isAction) continue;
      latest = {
        text: entry.message ?? '',
        mine: entry.playerIndex === gs.myPlayerIndex,
        id: entry.id,
      };
    }
    toastLogIdRef.current = gs.log[gs.log.length - 1].id;
    if (latest && latest.text) {
      setActionToast({ text: latest.text, mine: latest.mine, key: Date.now() });
      const t = setTimeout(() => setActionToast(null), 1400);
      return () => clearTimeout(t);
    }
  ...
  const validTargetIds = useMemo<Set<string>>(() => {
    ...
    if (targeting.type === 'attack') {
      ...
      const enemyHasTaunt = opBoard.some(
        (m) => hasTauntKeyword(m, getCard(m.cardCode))
      );
      const ids = new Set<string>();
      for (const m of opBoard) {
        if (m.hasStealthUntilAttack) continue;
        if (enemyHasTaunt) {
          if (hasTauntKeyword(m, getCard(m.cardCode))) {
  ...
  return (
    ...
    >
      ...
      {/* Action toast — appears above the acting hero so you always
          see what your opponent (or you) just did. Anchored top for
          opponent, bottom for self. z-[55] sits below the card showcase
          so the bigger callout wins when both fire. */}
      {actionToast && (
        <div
          key={actionToast.key}
          className={`pointer-events-none fixed left-1/2 -translate-x-1/2 z-[55] animate-float-damage`}
          style={{ top: actionToast.mine ? undefined : '14%', bottom: actionToast.mine ? '22%' : undefined }}
        >
          <div className={`px-4 py-1.5 rounded-full border-2 shadow-[0_6px_16px_rgba(0,0,0,0.6)] text-sm font-bold whitespace-nowrap
            ${actionToast.mine
              ? 'bg-gradient-to-b from-emerald-700 to-emerald-900 border-emerald-300 text-emerald-50'
              : 'bg-gradient-to-b from-red-800 to-red-950 border-red-300 text-red-50'}`}
          >
            {actionToast.text}
          </div>
        </div>
      ...
      <div className="flex flex-1 min-h-0 flex-col items-center px-2 md:px-4 pt-0.5 md:pt-1 pb-0">
        ...
        <div className="flex items-center justify-center w-full gap-2 md:gap-4 relative">
          {/* Opponent name + class label + mode badge — left side */}
          <div className="absolute left-16 top-1/2 -translate-y-1/2 hidden md:flex flex-col items-start gap-0.5">
            <span className="text-[10px] text-amber-300/90 font-bold uppercase tracking-wider drop-shadow-md bg-black/40 rounded px-2 py-px">
              {HERO_ARCHETYPE[gs.opponent.heroClass] ?? ''}
            </span>
      ...
      <div className="hidden md:flex absolute right-3 top-1/2 -translate-y-1/2 z-20 flex-col items-center gap-3">
        ...
        <div className="flex-1 min-h-[100px] flex items-stretch" style={{ transform: 'rotate(3deg)' }}>
          <DeckPile count={gs.opponentDeckCount} graveyardCount={gs.opponent.graveyardCount} cardBackId={(gs as any).opponentCardBack} />
        </div>
        ...
        <div className="flex-1 min-h-[100px] flex items-stretch" style={{ transform: 'rotate(-3deg)' }}>
          <DeckPile count={gs.deckCount} graveyardCount={gs.myGraveyardCount} cardBackId={(gs as any).myCardBack} />
        </div>
      </div>
      ...
      >
        ...
        {/* My hero row */}
        <div className="flex items-center justify-center w-full gap-2 md:gap-4 relative">
          {/* My name + class label + mode badge — left side */}
          <div className="absolute left-16 top-1/2 -translate-y-1/2 hidden md:flex flex-col items-start gap-0.5">
            <span className="text-[10px] text-amber-300/90 font-bold uppercase tracking-wider drop-shadow-md bg-black/40 rounded px-2 py-px">
              {HERO_ARCHETYPE[gs.myHeroClass] ?? ''}
            </span>
