 ...
 *   Des: Destroy 2 enemy minions
 *   Astrid: Have 3+ Divine Shield minions at once
 *   Ava: Summon 5+ minions total
 *   Lucas: Return 2+ enemy minions (upgradeProgress)
...
export function checkHeroPowerUpgrade(game: GameState, playerIndex: 0 | 1): void {
  ...
  switch (player.heroClass) {
    ...
    case 'ANDERS':
      ...
      break;
    case 'DES':
      if (stats.minionsKilled >= 2) shouldUpgrade = true;
      break;
    case 'ASTRID':
      if (player.board.filter(m => m.hasDivineShield).length >= 3) shouldUpgrade = true;
      break;
    case 'AVA':
      if (stats.minionsPlayed + stats.heroPowerUses >= 5) shouldUpgrade = true;
      break;
    case 'LUCAS':
      if (player.upgradeProgress >= 2) shouldUpgrade = true;
      break;
    case 'IZZY':
